...
while ($LASTEXITCODE -ne 0) {
    ...
    $attempt = $attempt + 1
    Write-Host "Round ${attempt} started - ${now}: Base image ltsc2025:vs22 not found locally. Building from Dockerfile..."
...
while ($LASTEXITCODE -ne 0) {
    $now = Get-Date -Format "yyyy-MM-dd HH:mm:ss"; 
    $attempt = $attempt + 1;
    Write-Host "Round ${attempt} started - ${now}: Base image ltsc2025:vs22.sql22 not found locally. Building from Dockerfile..."
    docker system prune -f
