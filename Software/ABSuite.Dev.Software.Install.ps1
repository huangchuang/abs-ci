...
function Install-MSOLEDBDriver19 {
    ...
    $arguments=@('/i', """C:\Software\MSOLEDB 19\msoledbsql.msi""", 'ADDLOCAL=ALL', '/quiet', 'IACCEPTMSOLEDBSQLLICENSETERMS=YES', '/norestart')
    $process = Start-Process -FilePath "C:\Windows\System32\msiexec.exe" -ArgumentList $arguments -PassThru -Wait -NoNewWindow
...
function Install-OpenJDK {
    ...
    Invoke-WebRequest -Uri $url -OutFile "C:\Software\$zip"
    Expand-Archive -Path "C:\Software\$zip" -DestinationPath "$env:TEMP" -Force
...
function Install-TortoiseSVN {
    ...
    $msi = "C:\Software\TortoiseSVN\TortoiseSVN-1.14.9.29743-x64-svn-1.14.5.msi"
...
function Install-UnisysOLEDBDriver {
    ...
    $arguments=@('/i', """C:\Software\UnisysOLEDB\OLEDBx64.msi""", 'ADDLOCAL=ALL', '/quiet', 'IACCEPTMSOLEDBSQLLICENSETERMS=YES', '/norestart')
    $process = Start-Process -FilePath "C:\Windows\System32\msiexec.exe" -ArgumentList $arguments -PassThru -Wait -NoNewWindow
    $process = Get-Process -Name msiexec -ErrorAction SilentlyContinue
    if ($process) { $process.WaitForExit() }
